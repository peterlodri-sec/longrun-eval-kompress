...
persisted by an asynchronous SQLite kernel. Algorithm~\ref{alg:fourphase}
...
\label{alg:fourphase}
\begin{algorithmic}[1]
\State \textbf{input:} hypothesis $h$, budget $B$, state $\Sigma$
\State \textbf{Phase 1: Plan}
  \State \quad $p \gets \mathrm{Plan}(h, \Sigma)$ \Comment{derive experiment spec from hypothesis + state}
\State \textbf{Phase 2: Execute}
  \State \quad $r \gets \mathrm{Execute}(p)$ \Comment{run training/eval per spec}
  \State \quad $\Sigma \gets \mathrm{SQLiteWrite}(\Sigma, p, r)$ \Comment{async persist}
\State \textbf{Phase 3: Evaluate}
  \State \quad $m \gets \mathrm{eval\_heretic}(r)$ \Comment{adversarial benchmark}
  \State \quad $\Sigma \gets \mathrm{SQLiteWrite}(\Sigma, m)$
\State \textbf{Phase 4: Decide}
  \State \quad $d \gets \mathrm{Council}(m, \Sigma, B)$ \Comment{LLM council: SHIP / RETRAIN / PIVOT}
  \State \quad \textbf{if} $d = \textsc{Ship}$ \textbf{then} \Return $r$
  \State \quad \textbf{if} $d = \textsc{Retrain}$ \textbf{then} $h \gets \mathrm{RefineHypothesis}(h, m)$
  \State \quad \textbf{if} $d = \textsc{Pivot}$ \textbf{then} $h \gets \mathrm{NewHypothesis}(m, \Sigma)$
\State \textbf{goto} Phase 1 \Comment{or halt if budget $B$ exhausted}
\end{algorithmic}
\end{algorithm}
...
\midrule
L1 & Agent        & \texttt{Loop.run()}              & training/eval iteration (Alg.~\ref{alg:fourphase}) \\
...
Algorithm~\ref{alg:fourphase}. The abstract \texttt{Loop} class in
...
Algorithm~\ref{tab:loops} to an asynchronous SQLite database, serving as the
